    ...
    <>
        ...
        <div className="container relative grid gap-12 py-20 md:grid-cols-2 md:py-32">
          <div className="flex flex-col justify-center animate-fade-in-up">
            ...
            <h1 className="font-display text-4xl font-bold leading-tight md:text-6xl">
              Faça a diferença <br /> com sua <span className="text-accent-glow">doação</span>
            </h1>
            <p className="mt-6 max-w-lg text-lg text-primary-foreground/80">
              Conectamos doadores a instituições de saúde que transformam a vida de milhares de brasileiros todos os dias.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <Button asChild size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground shadow-accent rounded-xl text-base">
                <Link to="/instituicoes">
                  Doar agora <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="rounded-xl border-white/30 bg-white/10 text-primary-foreground hover:bg-white/20 hover:text-primary-foreground">
      ...
      <section className="container py-20">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {[
            { icon: Shield, title: "100% Seguro", desc: "Pagamentos criptografados e dados protegidos." },
            { icon: HandHeart, title: "Impacto Real", desc: "Acompanhe o destino de cada contribuição." },
            { icon: Users, title: "Comunidade", desc: "Junte-se a milhares de apoiadores." },
          ].map((f) => (
            <div key={f.title} className="rounded-2xl border bg-card p-8 shadow-soft transition-smooth hover:-translate-y-1 hover:shadow-elegant">
      ...
      <section id="instituicoes" className="bg-secondary/30 py-20">
        <div className="container">
          <div className="mb-12 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
            <div>
              <h2 className="font-display text-3xl font-bold md:text-4xl">Instituições parceiras</h2>
              <p className="mt-2 text-muted-foreground">Escolha uma causa para apoiar agora mesmo.</p>
            </div>
            <Button asChild variant="outline" className="rounded-xl">
              <Link to="/instituicoes">Ver todas <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
          </div>
      ...
      <section className="container py-20">
        <div className="relative overflow-hidden rounded-3xl gradient-primary p-12 text-center text-primary-foreground md:p-16">
          <h2 className="font-display text-3xl font-bold md:text-4xl">Sua doação salva vidas</h2>
          <p className="mx-auto mt-4 max-w-xl text-primary-foreground/80">
            Em poucos cliques, você pode mudar a história de uma família.
          </p>
          <Button asChild size="lg" className="mt-8 rounded-xl bg-accent hover:bg-accent/90 text-accent-foreground shadow-accent">
